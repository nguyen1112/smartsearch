"""
SmartSearch - Advanced file search engine powered by AI
...
from smart_search.core.frontend import setup_frontend_routes
from smart_search.core.initialization import (
    critical_init,
    health_monitoring_loop,
)
from smart_search.core.logging import logger
from smart_search.core.telemetry import telemetry
from smart_search.services.crawler.manager import get_crawl_job_manager
...
def startup_handler():
    ...
    try:
        ...
        if settings.debug:
            ...
            # frontend is at apps/smartsearch/frontend, main.py is at apps/smartsearch/smart_search/main.py
...
def perform_shutdown(vite_process=None):
    ...
    try:
        ...
        try:
            from smart_search.services.typesense_client import get_typesense_client
        ...
        from smart_search.services.docker_manager import get_docker_manager
...
if not frontend_dist_path:
    try:
        ...
        # Check resources in the 'smart_search.frontend' subpackage
        frontend_pkg_path = files("smart_search.frontend") / "dist"
...
def health_check():
    ...
    from smart_search.services.service_manager import get_service_manager
...
def cli_main():
    ...
    from smart_search.lib.flaskwebgui import FlaskUI
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run SmartSearch application")
    parser.add_argument(
        "--mode",
        choices=["dev", "prod"],
        default=None,
    ...
    if settings.debug:
        uvicorn.run("smart_search.main:app", host=settings.host, port=port, reload=True, log_level="info")
    else:
        from smart_search.lib.flaskwebgui import FlaskUI
