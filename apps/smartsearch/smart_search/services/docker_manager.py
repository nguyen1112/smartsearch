...
class DockerManager:
    """Manages docker-compose services for SmartSearch"""
    ...
    def __init__(self, compose_file_path: Optional[str] = None, use_gpu: Optional[bool] = None):
        ...
        if use_gpu is None:
            from smart_search.utils.gpu_detector import should_use_gpu_mode
        ...
        if compose_file_path:
            ...
            # Locating docker-compose.yml (now consistently in smart_search/docker-compose.yml)
            try:
                ...
                # Check inside the smart_search package (installed mode or proper package structure)
                pkg_compose = files("smart_search") / compose_filename
                if pkg_compose.is_file():
                    self.compose_file = Path(pkg_compose)
                else:
                    # Fallback for dev/source mode
                    # Location: smart_search/services/docker_manager.py -> smart_search/docker-compose.yml
                    # Go up 2 levels: services -> smart_search
